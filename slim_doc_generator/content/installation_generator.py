...
class InstallationGenerator:
    ...
    def _generate_installation_by_type(self, content: List[str], repo_info: Dict, 
        ...
        Args:
            ...
            repo_info: Repository information dictionary
            has_npm: Whether the repository has package.json
            has_pip: Whether the repository has setup.py or requirements.txt
            has_docker: Whether the repository has Dockerfile or docker-compose.yml
        """
        ...
        if has_npm:
            ...
            self._add_npm_scripts(content, repo_info)
        ...
        if has_pip:
            ...
            if "requirements.txt" in repo_info["files"]:
                ...
                content.append(f"git clone {repo_url}\n")
                content.append(f"cd {os.path.basename(project_name)}\n\n")
                content.append("# Create and activate a virtual environment (optional but recommended)\n")
    ...
    def _add_npm_scripts(self, content: List[str], repo_info: Dict) -> None:
        """
        Add npm scripts information if available.
        ...
        try:
            ...
            if 'scripts' in package_data and package_data['scripts']:
                ...
                content.append("The project includes the following npm scripts:\n")
                
                for script, command in package_data['scripts'].items():
                    if script in ['start', 'build', 'test', 'dev', 'serve', 'deploy']:
                        content.append(f"- `npm run {script}`: {command}")
                
                content.append("\n")
        
        except Exception as e:
            self.logger.warning(f"Error extracting npm scripts: {str(e)}")
